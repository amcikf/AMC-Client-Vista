...
const SMART_RULES = [
  ...
  { keywords: ["blog update"], minutes: 45 },
...
async function drawReferenceStyleClientPdfHeader(doc, report, { logoDataUrl = "", reportPeriodLabel = "" } = {}) {
  ...
  const statAccent = [214, 166, 76];
  const statLabelColor = [89, 102, 117];
  const statValueColor = [29, 40, 54];
  const statDescColor = [110, 122, 135];
...
async function registerServiceWorker() {
  ...
  try {
    await navigator.serviceWorker.register("./sw.js?v=20260453");
  } catch (error) {
